...
See the [isolated fixture guide](../../.agents/skills/test-t3-app/references/sqlite-fixtures.md) for preparing test data.
...
Shared runs use standard dev serving by default. Set `T3CODE_BUNDLED_DEV=1` to opt into
bundled dev when network round trips dominate startup. Two reload traps matter
...
Run the workspace `vp check` and `vp run typecheck` before completing a change.
Use `vp run lint:mobile` for native mobile changes. See
