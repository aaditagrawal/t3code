...
## Fork adapter boundaries

The [driver registry](../../apps/server/src/provider/builtInDrivers.ts) includes the fork
providers. Cursor uses its SDK rather than upstream ACP; transport-specific upstream fixes
need equivalent SDK handling. Keep SDK models, account configuration, and usage events intact.
Do not infer conversation rewind from filesystem checkpoint support.

Legacy provider names remain decodable when replaying persisted fork events. New aggregate
replay queries must use the same normalization and boundary redaction as global replay.
Migration identifiers retain the deployed fork lineage even where upstream uses different IDs.
