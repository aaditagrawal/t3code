...
Bind direct servers to a trusted LAN or tailnet interface. Custom process launchers can pass
their authentication token through `--bootstrap-fd` rather than command-line arguments; see
[authentication constraints](../internals/environment-auth.md) for the server boundary.
