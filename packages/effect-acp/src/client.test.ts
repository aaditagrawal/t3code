...
it.effect("preserves provider form metadata for a registered SDK elicitation extension", () =>
  Effect.gen(function* () {
    const { stdio, input, output } = yield* makeInMemoryStdio();
    const acp = yield* AcpClient.make(stdio);
    const params = {
      ...
      requestedSchema: {
        ...
        properties: {
          choice: {
            type: "string",
            oneOf: [{ const: "yes", title: "Yes", description: "Keep this detail" }],
          },
        },
      },
    };
    yield* acp.handleElicitation(() => Effect.die("core handler shadowed extension"));
    yield* acp.handleExtRequest("elicitation/create", Schema.Unknown, (request) => {
      assert.deepEqual(request, params);
      return Effect.succeed({ action: "accept", content: { choice: "yes" } });
    });
    yield* Queue.offer(
      input,
      yield* encodeJsonl(jsonRpcRequest("elicitation/create", Schema.Unknown), {
