  ...
  const clientHandlerLayer = AcpRpcs.CompatClientRpcs.toLayer(
    AcpRpcs.CompatClientRpcs.of({
      ...
      [CLIENT_METHODS.elicitation_create]: (payload, { requestId }) => {
        const method = CLIENT_METHODS.elicitation_create;
        const context = requestContext(requestId, method);
        const extension = extRequestHandlers.get(method);
        if (extension) {
          return extension(payload, context).pipe(
            ...
            Effect.flatMap((response) =>
              Schema.decodeUnknownEffect(
                Schema.Union([
                  AcpSchemaV2.CreateElicitationResponse,
                  AcpSchemaV1.CreateElicitationResponse,
                ]),
              )(response).pipe(
                Effect.mapError((cause) =>
                  AcpError.AcpRequestError.invalidExtensionPayload(method, cause).toProtocolError(),
                ),
              ),
            ),
          );
