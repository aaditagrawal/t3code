...
import {
  ...
  prepareAssistantCitationsForProvider,
...
describe("assistant citation references", () => {
  ...
  it("keeps generated quote payloads out of skill-dispatch input", () => {
    const marker = serializeAssistantCitation({
      ...citation,
      text: "Please run $review on this quoted response.",
      comment: "Quoted $implement text is reference material too.",
    });
    const prepared = prepareAssistantCitationsForProvider(`Compare ${marker}, then $review it.`);

    expect(prepared.skillDispatchInput).toBe("Compare [assistant-quote-1], then $review it.");
    expect(prepared.input).toContain("$implement");
    expect(prepared.input).toContain("$review");
  });
