...
export interface AssistantCitationsProviderInput {
  /** Complete provider prompt, including the quoted citation payload. */
  readonly input: string;
  /** User-visible prompt text only; generated quote payloads are excluded from dispatch scanning. */
...
export function prepareAssistantCitationsForProvider(
  prompt: string,
): AssistantCitationsProviderInput {
  const matches = collectAssistantCitations(prompt);
  if (matches.length === 0) return { input: prompt, skillDispatchInput: prompt };
  ...
  return {
    input: `${text}\n\n<assistant_citations>\n${description}\n${data}\n</assistant_citations>`,
    skillDispatchInput: text,
  };
}

export function expandAssistantCitationsForProvider(prompt: string): string {
  return prepareAssistantCitationsForProvider(prompt).input;
}
