...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    ...
    assert.include(
      script,
      "T3_RELEASE_BASE_URL='https://github.com/aaditagrawal/t3code/releases/download'",
    );
    assert.include(
      script,
      'T3_RUNTIME_DIR="$HOME/.t3code-fork/runtime/versions/$T3_ARCHIVE_VERSION"',
    );
    ...
    assert.include(
      script,
      'T3_LOCK="$HOME/.t3code-fork/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"',
    );
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const versionsDir = `${home}/${HOME_DIR_NAME}/runtime/versions`;
