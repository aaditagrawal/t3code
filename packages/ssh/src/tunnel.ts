import { DEFAULT_SERVER_PORT, HOME_DIR_NAME } from "@t3tools/shared/branding";
import type {
...
const DEFAULT_REMOTE_PORT = DEFAULT_SERVER_PORT;
...
T3_RUNTIME_DIR="$HOME/${HOME_DIR_NAME}/runtime/versions/$T3_ARCHIVE_VERSION"
t3_runtime_ready() {
  [ -x "$T3_RUNTIME_DIR/t3" ] && [ "$(cat "$T3_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$T3_ARCHIVE_VERSION" ]
}
if ! t3_runtime_ready; then
  mkdir -p "$HOME/${HOME_DIR_NAME}/runtime/versions"
  ...
  # under it.
  T3_LOCK="$HOME/${HOME_DIR_NAME}/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_STAGING="$(mktemp -d "$HOME/${HOME_DIR_NAME}/runtime/versions/.staging-XXXXXX")"
