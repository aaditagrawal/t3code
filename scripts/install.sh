...
t3_home="${T3CODE_HOME:-$HOME/.t3code-fork}"
...
case "$(uname -s)" in
  ...
  *) fail "unsupported operating system $(uname -s); use the desktop app or the PowerShell installer" ;;
esac
case "$(uname -m)" in
...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$version" ]; then
  ...
  if [ "$fetch_status" -eq 44 ]; then
    fail "t3 ${version} has no release archive for ${platform}-${arch}; choose a newer fork release or build this version from source"
