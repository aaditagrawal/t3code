...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    // Copilot is a spawned executable, so it must be explicitly unpacked on
    // platforms where the server remains in app.asar. The Windows copy lives
    // in server.asar, whose executable glob is unpacked while packing.
    ...(platform === "win" ? {} : { asarUnpack: ["node_modules/@github/copilot*/**/*"] as const }),
    extraResources: [
