  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "@aaditagrawal/t3code-darwin-arm64",
          "@aaditagrawal/t3code-linux-x64",
          "@aaditagrawal/t3code",
        ],
      );
      ...
      const linuxDir = path.join(fixture.outputDir, "@aaditagrawal/t3code-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@aaditagrawal/t3code-linux-x64");
      ...
      assert.deepStrictEqual(launcherManifest.optionalDependencies, {
        "@aaditagrawal/t3code-darwin-arm64": VERSION,
        "@aaditagrawal/t3code-linux-x64": VERSION,
      });
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@aaditagrawal"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@aaditagrawal/t3code-linux-x64.tgz")],
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        ...
        const unpack = yield* run(
          ...
          [
            "-xf",
            path.join(fixture.outputDir, "@aaditagrawal/t3code.tgz"),
            "-C",
            installedLauncher,
          ],
          {
      ...
      assert.include(unsupported.stderr, "https://github.com/aaditagrawal/t3code/releases");
    }),
  );
});
