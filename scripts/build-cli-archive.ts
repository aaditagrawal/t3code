...
import { resolveBundledCopilotPlatformPackages } from "./lib/copilot-platform-packages.ts";
...
const stageRuntimeExternals = Effect.fn("stageRuntimeExternals")(function* (input: {
  ...
  const copilotVersion = serverDependencies["@github/copilot"];
  if (copilotVersion === undefined || copilotVersion.trim().length === 0) {
    return yield* new CliArchiveInputMissingError({
      inputPath: "apps/server/package.json#dependencies['@github/copilot']",
      hint: "The archive stages Copilot's platform executable from this version.",
    });
  }
  const dependencies = {
    ...Object.fromEntries(
      resolveBundledCopilotPlatformPackages(input.platform, input.arch).map((name) => [
        name,
        copilotVersion,
      ]),
    ),
