...
#   T3CODE_HOME              T3 home directory (default: ~\.t3code-fork)
#   T3CODE_INSTALL_BIN_DIR   where t3f.exe is linked (default: ~\.local\bin)
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    try {
      ...
      if ($status -eq 404) {
        Fail "t3 $version has no release archive for win32-$arch; choose a newer fork release or build this version from source"
      }
      throw
    }
