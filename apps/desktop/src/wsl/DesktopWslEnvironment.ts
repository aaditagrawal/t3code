...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    "set -eu",
    `runtime_parent="$HOME/${HOME_DIR_NAME}/wsl-runtime"`,
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/${HOME_DIR_NAME}/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
const RESOLVED_ENV_LINES = [
  `for name in ${NODE_PTY_PROBE_ENV_NAMES.join(" ")}; do`,
  '  value=$(printenv "$name" 2>/dev/null) || continue',
  '  [ -n "$value" ] || continue',
  '  encoded=$(printf "%s" "$value" | base64 | tr -d "\\r\\n")',
  `  printf 'resolvedEnv:%s=%s\\n' "$name" "$encoded"`,
  "done",
].join("\n");
...
export const buildWslRuntimeProbeScript = (linuxAppRoot: string) =>
  [
    `bash -lc ${shellQuote(`${buildWslNodeEnvPreamble()}${RESOLVED_PATH_LINE}\n${RESOLVED_ENV_LINES}`)} 2>/dev/null || ${RESOLVED_PATH_LINE}`,
...
const probeWslRuntimeImpl = (
  ...
  Effect.gen(function* () {
    ...
    return { ok: true, resolvedPath, resolvedEnv: parseResolvedEnv(probe.stdout) } as const;
  });
