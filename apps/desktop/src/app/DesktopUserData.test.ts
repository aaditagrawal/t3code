...
it.effect("identifies a failed source read and preserves its cause", () => {
  const sourceState = "/profiles/t3code-fork/Local State";
...
for (const sourceName of ["t3code-fork", "T3 Code Fork (Alpha)"]) {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const destination = path.join(directory, "t3code-fork-v2");
        const state = '{"os_crypt":{"encrypted_key":"test-encrypted-key"}}';
        yield* fs.makeDirectory(path.join(directory, "T3 Code Fork (Alpha)"), { recursive: true });
