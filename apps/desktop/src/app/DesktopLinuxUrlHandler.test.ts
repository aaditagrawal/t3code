...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    ...
    assert.include(entry, `MimeType=x-scheme-handler/${URL_SCHEME};`);
  });
  ...
  it("carries structured context on registration errors", () => {
    ...
    assert.equal(
      writeError.message,
      `Failed to register the ${URL_SCHEME}:// URL handler (step: write-desktop-entry).`,
    );
    assert.equal(
    ...
    assert.equal(
      exitError.message,
      `Failed to register the ${URL_SCHEME}:// URL handler (step: set-default-handler, xdg-mime exit code 4).`,
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(recorded.files[0]?.content, `MimeType=x-scheme-handler/${URL_SCHEME};`);
