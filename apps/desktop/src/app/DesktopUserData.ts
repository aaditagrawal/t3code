...
export const resolveUserDataPath = Effect.fn("desktop.userData.resolveUserDataPath")(
  function* (input: {
    ...
    const names = input.isDevelopment
      ? { current: DESKTOP_USER_DATA_DIR_NAME_DEV, legacy: "T3 Code Fork (Dev)" }
      : { current: `${DESKTOP_USER_DATA_DIR_NAME}-v2`, legacy: DESKTOP_USER_DATA_DIR_NAME };
    ...
    const sourceState = (yield* inspect(legacyState))
      ? legacyState
      : path.join(input.appDataDirectory, "T3 Code Fork (Alpha)", "Local State");
