...
/** True when a card row draws a status word ("Approval", "Done", …) in the
    timestamp slot. Waiting has no word, so those cards still draw a time. */
function threadListV2CardOwnsStatusSlot(
  thread: Parameters<typeof resolveThreadListV2Status>[0] &
    Parameters<typeof threadHasUnseenCompletion>[0],
): boolean {
  const status = resolveThreadListV2Status(thread);
  if (status === "waiting") return false;
  if (status === "ready") return threadHasUnseenCompletion(thread);
  return true;
}
...
function resolveThreadListV2ItemTimeLabel(
  ...
  // Waiting cards have no status label, so they still draw the timestamp.
  // Unread completions draw "Done" in that slot instead of a time.
  if (variant === "card" && threadListV2CardOwnsStatusSlot(thread)) return "";
