...
export function planClaudeSkillDispatch(
  ...
  // ProviderService guarantees that the dispatch input is a prefix of the
  // enriched provider prompt. Fall back to the scanned text if an internal
  // caller violates that invariant so generated context is never scanned.
  const outputPrompt = providerPrompt.startsWith(prompt) ? providerPrompt : prompt;
  const leading = outputPrompt.slice(0, last.start);
  const trailing = outputPrompt.slice(last.end);
