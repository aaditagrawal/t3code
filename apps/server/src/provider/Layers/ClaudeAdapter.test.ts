...
import {
  ...
  SYNTHETIC_CLAUDE_GATEWAY_MODEL,
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("does not dispatch a skill mentioned only in generated provider context", () => {
    const homeDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "claude-skills-quoted-"));
    NodeFS.mkdirSync(NodePath.join(homeDir, "skills", "review"), { recursive: true });
    NodeFS.writeFileSync(
      NodePath.join(homeDir, "skills", "review", "SKILL.md"),
      "---\ndescription: Review.\n---\n# Body\n",
    ...
    return Effect.gen(function* () {
      ...
      const input =
        "Compare [assistant-quote-1].\n\n<assistant_citations>\nQuoted $review request\n</assistant_citations>";
      yield* adapter.sendTurn({
        threadId: session.threadId,
        input,
        skillDispatchInput: "Compare [assistant-quote-1].",
        attachments: [],
      });
  ...
  it.effect("seeds Claude Sonnet 5 sessions with the default 200K context window", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
  ...
  it.effect("seeds Claude Sonnet 5 sessions with 1M when that context window is selected", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      harness.query.emit({
        type: "stream_event",
        session_id: "sdk-session-sonnet-5-1m-usage",
        uuid: "stream-sonnet-5-1m-usage",
        parent_tool_use_id: null,
        event: {
          type: "message_delta",
          delta: {},
  ...
  it.effect("honors an explicit Claude Sonnet 5 200K context window selection", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
