...
it.effect("materializes the fresh-install default Hermes slot when enrolling", () =>
  Effect.gen(function* () {
    ...
    const enrollment = yield* broker.createEnrollment({
      ...
      connectorUrl: "https://t3.example.test/api/hermes-gateway/ws",
    });

    assert.equal(enrollment.instanceId, defaultHermesInstanceId);
    const configured = (yield* settings.getSettings).providerInstances[defaultHermesInstanceId];
    assert.equal(configured?.driver, "hermes");
    assert.equal(configured?.displayName, "Default Hermes");
    assert.deepInclude(configured?.config as object, {
      binaryPath: "hermes-acp",
      connectorUrl: "https://t3.example.test/api/hermes-gateway/ws",
      revoked: false,
    });
  }).pipe(Effect.provide(Layer.mergeAll(ServerSettings.layerTest(), NodeServices.layer))),
);
...
it.effect("invalidates the previous credential when replacement enrollment begins", () =>
  Effect.gen(function* () {
    ...
    const staleDelivery = yield* Effect.flip(
      broker.withAuthorizedConnection(delivery, Effect.void),
    );
    assert.include(staleDelivery.detail, "no longer authorized");
...
it.effect("stops the displaced generation's ping loop on reconnect", () =>
  Effect.gen(function* () {
    const broker = yield* makeBroker(makeSecretStore());
    const enrollment = yield* enroll(broker, instanceId, "Reconnected Hermes");
    const first = recordingTransport();
    const initial = yield* broker.registerConnection(
      hello({ type: "enrollment-token", token: enrollment.oneTimeToken }),
      first.transport,
    );
    const credential = initial.accepted.credential;
    if (!credential) return yield* Effect.die(new Error("enrollment did not issue a credential"));
    ...
    const displacedFrameCount = first.sent.length;

    yield* TestClock.adjust(Duration.seconds(11));
    // The new generation starts its liveness probe. The old generation's
    // child scope was closed during replacement, so it never probes again.
    assert.isTrue(replacement.sent.some((message) => message.type === "ping"));
    assert.equal(first.sent.length, displacedFrameCount);
  }).pipe(Effect.provide(testLayer)),
);
...
it.effect("preserves the ACP provider and concurrent settings edits during companion removal", () =>
  Effect.gen(function* () {
    const broker = yield* makeBroker(makeSecretStore());
    const settings = yield* ServerSettings.ServerSettingsService;
    ...
    assert.equal(finalSettings.providerInstances[instanceId]?.driver, "hermes");
    assert.equal(finalSettings.providerInstances[instanceId]?.displayName, "Concurrent Research");
    assert.deepEqual(finalSettings.providerInstances[instanceId]?.config, {});
  }).pipe(Effect.provide(testLayer)),
);
...
it.effect("requires revocation before removing a companion enrollment", () =>
  Effect.gen(function* () {
    const secrets = makeSecretStore();
    const broker = yield* makeBroker(secrets);
    const settings = yield* ServerSettings.ServerSettingsService;
    ...
    const configured = (yield* settings.getSettings).providerInstances[instanceId];
    assert.equal(configured?.driver, "hermes");
    assert.equal(configured?.displayName, "Disposable Hermes");
    assert.deepEqual(configured?.config, { binaryPath: "/opt/hermes-acp" });
    assert.isFalse(
...
it.effect("leaves an unenrolled Hermes ACP provider configured", () =>
  Effect.gen(function* () {
    const broker = yield* makeBroker(makeSecretStore());
    const settings = yield* ServerSettings.ServerSettingsService;

    assert.deepEqual(yield* broker.removeInstance(instanceId), { instanceId });
    assert.equal((yield* settings.getSettings).providerInstances[instanceId]?.driver, "hermes");
    assert.equal(
  ...
  Effect.gen(function* () {
    ...
    yield* secrets.set(
      ...
      legacyBlob({
        ...
        connectorUrl: "https://t3.example.test:7446/api/hermes-gateway/ws",
        revoked: true,
        removed: true,
      }),
    );
...
it.effect("a delivery connection never displaces the live gateway connection", () =>
  Effect.gen(function* () {
    ...
    yield* broker.revokeInstance(instanceId);
    const revokedDelivery = yield* Effect.flip(
      broker.withAuthorizedConnection(delivery, Effect.void),
    );
