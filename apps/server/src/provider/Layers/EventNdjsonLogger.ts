...
function resolveStreamLabel(stream: EventNdjsonStream): string {
  return stream === "native" ? "NATIVE" : stream === "orchestration" ? "ORCH" : "CANON";
}
...
function isProviderLogFile(filePath: string, fileName: string, filePrefix: string): boolean {
  ...
  try {
    ...
    // NTIVE is the pre-fork native tag; keep accepting it so log files written
    // before the label widened to NATIVE still validate.
    return /^\[[^\]\r\n]+\] (?:NATIVE|NTIVE|CANON|ORCH): /u.test(
