...
export interface StandardAcpAdapterConfig<UserInputParams = never, UserInputEncoded = never> {
  ...
  readonly currentModelFromSetup: (
    setup:
  ...
  /** Cancel the active ACP prompt before dispatching a mid-turn replacement. */
  readonly cancelInFlightPromptOnSteer?: boolean;
}
...
const resolveNotificationTurnId = (ctx: StandardAcpSessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: StandardAcpSessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, StandardAcpSessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
export function makeStandardAcpAdapter<UserInputParams = never, UserInputEncoded = never>(
  config: StandardAcpAdapterConfig<UserInputParams, UserInputEncoded>,
  ...
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: `Failed to generate ${config.label} runtime identifier.`,
            cause,
          }),
      ),
    );
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: `Failed to process ${config.label} ACP callback.`,
              cause,
            }),
        ),
      );
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = standardAcpPromptSettlementBelongsToContext({
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning(`Failed to write native ${config.label} notification log.`, {
            cause,
            threadId,
            method,
          }),
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const session: ProviderSession = {
            ...
            runtimeMode: input.runtimeMode,
            cwd,
            ...(boundModelId ? { model: config.normalizeModel(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          const ctx: StandardAcpSessionContext = {
            threadId: input.threadId,
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError(`Failed to process ${config.label} runtime notification.`, { cause }),
            ),
            // The consumer must outlive startSession's calling fiber. Its
            // session scope is retained on the context and closed on teardown.
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: `${config.label} ACP session ready` },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
    ...
    const sendTurn: StandardAcpAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: `${config.label} prompt was interrupted during preparation.`,
                });
              }
              ...
              } else if (config.cancelInFlightPromptOnSteer) {
                // Do not discard the live prompt until the replacement has
                // completed preparation. A failed steer must leave it intact.
                yield* settlePendingApprovalsAsCancelled(ctx.pendingApprovals);
                yield* settlePendingUserInputsAsCancelled(ctx.pendingUserInputs);
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: `${config.label} prompt preparation failed.`,
                    emitTurnCompletion: false,
                  });
                }),
              ),
        ...
        return yield* Effect.gen(function* () {
          const promptStart = config.cancelInFlightPromptOnSteer
            ? yield* prepared.promptLifecycle.withPermit(
                Effect.gen(function* () {
                  const liveCtx = sessions.get(input.threadId);
            ...
            : ({ _tag: "Direct" as const } satisfies { readonly _tag: "Direct" });
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  prepared.acpSessionId,
                  {
                    errorMessage: `${config.label} session changed before the turn completed.`,
                    settleAllPrompts: true,
                  },
                );
                yield* Ref.set(promptSettled, true);
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: `${config.label} session changed before the turn completed.`,
                });
              }
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? `${config.label} prompt request failed.`,
                }),
              );
    ...
    const interruptTurn: StandardAcpAdapterShape["interruptTurn"] = (threadId, turnId) =>
      Effect.gen(function* () {
    ...
    const rollbackThread: StandardAcpAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: `${config.label} ACP sessions do not support provider-side rollback yet.`,
        });
      });
