  ...
  Effect.gen(function* () {
    ...
    const startOnce = Effect.gen(function* () {
      ...
      const authenticateAfterRequired = (
        ...
        Effect.gen(function* () {
          const configuredAuthMethodId = resolvedAuthMethodId?.trim();
      ...
      if (
        resolvedAuthMethodId !== undefined &&
        (options.authenticateEagerly === true || typeof options.authMethodId === "function")
      ) {
...
const handleSessionUpdate = ({
  ...
  Effect.gen(function* () {
    ...
    for (const event of parsed.events) {
      if (event._tag === "CommandsUpdated" || event._tag === "AvailableCommandsUpdated") {
        yield* recordAvailableCommands({
          events: [event],
          availableCommandsRef,
          availableCommandsReady,
...
const recordAvailableCommands = Effect.fn("AcpSessionRuntime.recordAvailableCommands")(function* ({
  ...
  for (const event of events) {
    ...
    if (event._tag !== "AvailableCommandsUpdated") continue;
    const commands = toAcpAvailableCommands(event.availableCommands);
    yield* Ref.set(availableCommandsRef, commands);
    yield* Deferred.succeed(availableCommandsReady, commands).pipe(Effect.asVoid);
  }
});
