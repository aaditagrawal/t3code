...
export interface ProjectDeleteInput {
  ...
  readonly expectedUnarchivedThreadIds?: ReadonlyArray<ThreadId>;
}
...
export class ProjectNotEmptyError extends Schema.TaggedError<ProjectNotEmptyError>()(
  ...
  { projectId: ProjectId, reason: Schema.optional(Schema.Literal("threads-changed")) },
) {
  override get message(): string {
    if (this.reason === "threads-changed")
      return `Project ${this.projectId} threads changed after deletion was confirmed.`;
...
export const make = Effect.gen(function* () {
  ...
  const deleteProject: ProjectService["Service"]["delete"] = Effect.fn("ProjectService.delete")(
    function* (input) {
      ...
      if (input.expectedUnarchivedThreadIds !== undefined) {
        const expectedIds = new Set(input.expectedUnarchivedThreadIds);
        const activeThreads = projectThreads.filter((thread) => thread.archivedAt === null);
        if (
          expectedIds.size !== input.expectedUnarchivedThreadIds.length ||
          activeThreads.some((thread) => !expectedIds.has(thread.id))
        ) {
          return yield* new ProjectNotEmptyError({ projectId, reason: "threads-changed" });
        }
        ...
        for (const threadId of expectedIds) {
          ...
          // A retry may find children already deleted by this same cascade. Only its
          // accepted receipt explains a missing confirmed thread; unrelated changes do not.
          const receipt = yield* commandReceipts
            .getByCommandId(CommandId.make(`${input.commandId}:delete-thread:${threadId}`))
            .pipe(
              Effect.mapError(
                (cause) =>
                  new ProjectOperationError({
          ...
          if (
            ...
            receipt.value.commandType !== "thread.delete"
          ) {
            return yield* new ProjectNotEmptyError({ projectId, reason: "threads-changed" });
          }
        }
      } else if (projectThreads.length > 0 && input.force !== true) {
      ...
      return yield* dispatch(
        projectId,
        {
          ...
          commandId: input.commandId,
          projectId,
          // The V2 roster was validated above and its children are now deleted.
          // Legacy projection rows can remain until this project command cascades them.
          ...(input.expectedUnarchivedThreadIds !== undefined
            ? { force: true }
...
export const layer = Layer.effect(ProjectService, make).pipe(
  ...
  Layer.provide(commandReceiptStoreLayer),
);
