...
export const make = Effect.gen(function* () {
  ...
  const findExistingFile = Effect.fn("ProjectFaviconResolver.findExistingFile")(function* (
    ...
    for (const relativePath of relativeCandidates) {
      ...
      // Resolve workspace candidates to guard against symlinks that escape the
      // project directory (e.g. favicon.svg -> /etc/passwd). A saved absolute
      // override is intentionally filesystem-scoped and is later authorized as
      // one exact canonical file by AssetAccess.
      const realCandidate = yield* fileSystem
        .realPath(candidate.value.absolutePath)
        .pipe(Effect.orElseSucceed(() => null));
      if (
        !realCandidate ||
        (candidateScope === "workspace" && !isPathWithinProject(realProjectCwd, realCandidate))
      ) {
        continue;
      }
