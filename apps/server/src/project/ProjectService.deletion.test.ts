...
import { layer as commandReceiptStoreLayer } from "../orchestration-v2/CommandReceiptStore.ts";
...
it.effect(
  "rejects deletion when a thread arrives after confirmation without deleting any children",
  () =>
    Effect.gen(function* () {
      const projectId = ProjectId.make("project:confirmation-race");
      const first = ThreadId.make("thread:confirmed");
      const arrived = ThreadId.make("thread:arrived-after-confirmation");
      yield* seedProject(projectId);
      yield* Effect.gen(function* () {
        const eventSink = yield* EventSinkV2;
        const projections = yield* ProjectionStoreV2;
        const service = yield* ProjectService.make;
        yield* eventSink.write({ events: [nativeThreadCreated(projectId, first)] });
        const confirmedIds = [first];
        yield* eventSink.write({ events: [nativeThreadCreated(projectId, arrived)] });
        const rejected = yield* service
          .delete({
            commandId: CommandId.make("command:stale-project-confirmation"),
            projectId,
            force: true,
            expectedUnarchivedThreadIds: confirmedIds,
          })
          .pipe(Effect.flip);
        ...
        assert.isNull((yield* projections.getThreadProjection(first)).thread.deletedAt);
        assert.isNull((yield* projections.getThreadProjection(arrived)).thread.deletedAt);
        const deleted = yield* service.delete({
          commandId: CommandId.make("command:renewed-project-confirmation"),
          projectId,
          expectedUnarchivedThreadIds: [first, arrived],
        });
        assert.isNotNull(deleted.deletedAt);
        assert.isNotNull((yield* projections.getThreadProjection(first)).thread.deletedAt);
        assert.isNotNull((yield* projections.getThreadProjection(arrived)).thread.deletedAt);
...
it.effect("retries a partial project deletion without repeating child events or cleanup", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      const input = { commandId, projectId, expectedUnarchivedThreadIds: threadIds };
