...
// Config directories and legacy HOME overrides are distinct persisted layouts.
const makeClaudeConfigDir = Effect.fn("AgentSessionScanner.test.makeClaudeConfigDir")(function* (
  prefix: string,
) {
  const fileSystem = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const root = yield* makeTempDir(prefix);
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("finds transcripts under the fork's legacy Claude HOME override", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const configDir = yield* makeClaudeConfigDir("t3code-claude-legacy-home-");
        const codexHomePath = yield* makeTempDir("t3code-codex-home-");
        const workspace = yield* makeTempDir("t3code-workspace-");
        yield* writeTranscript({
          filePath: path.join(configDir, "projects", "legacy", "session.jsonl"),
          contents: claudeSessionLine(workspace),
          mtimeMs: Date.parse("2026-01-01T00:00:00.000Z"),
        });
        const result = yield* runScan({ claudeHomePath: path.dirname(configDir), codexHomePath });
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeClaudeConfigDir("t3code-claude-legacy-");
        const codexHomePath = yield* makeTempDir("t3code-codex-legacy-");
        const claudeInstanceHome = yield* makeClaudeConfigDir("t3code-claude-instance-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeClaudeConfigDir("t3code-claude-legacy-");
        const codexHomePath = yield* makeTempDir("t3code-codex-legacy-");
        const claudeEnvironmentHome = yield* makeClaudeConfigDir("t3code-claude-env-");
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeClaudeConfigDir("t3code-metadata-home-");
        const secondHome = yield* makeClaudeConfigDir("t3code-metadata-second-");
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeClaudeConfigDir("t3code-short-metadata-home-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeClaudeConfigDir("t3code-record-metadata-home-");
        const secondHome = yield* makeClaudeConfigDir("t3code-record-metadata-second-");
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeClaudeConfigDir("t3code-directory-budget-home-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeClaudeConfigDir("t3code-record-limit-claude-");
    ...
    it.effect("shares a 64 MiB full-read budget across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeClaudeConfigDir("t3code-budget-claude-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeClaudeConfigDir("t3code-record-budget-claude-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeClaudeConfigDir("t3code-completed-claude-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const recentHome = yield* makeClaudeConfigDir("t3code-claude-recent-home-");
