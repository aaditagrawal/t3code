...
describe("attachmentStore", () => {
  ...
  it("preserves safe file extension metadata while using opaque storage", () => {
    ...
    expect(attachmentFileExtension("report.PDF")).toBe(".bin");
  ...
  it("resolves generic attachments without scanning the attachment directory", () => {
    ...
    try {
      ...
      const archivePath = NodePath.join(attachmentsDir, `${attachmentId}.bin`);
