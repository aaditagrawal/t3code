...
import { findWindowsShim, repointLauncher, resolveLauncherPath } from "./update.ts";

it.layer(NodeServices.layer)("t3 update launcher", (it) => {
  it.effect("finds only the fork Windows launcher for this executable", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "t3-update-windows-" });
      const executable = path.join(root, "runtime", "t3.exe");
      const shim = path.join(root, "t3f.cmd");
      yield* fs.writeFileString(path.join(root, "t3.cmd"), `@echo off\n"${executable}" %*\n`);
      const find = findWindowsShim(executable).pipe(
        Effect.provideService(HostProcessEnvironment, { PATH: root }),
      );
      assert.equal(yield* find, undefined);
      yield* fs.writeFileString(shim, `@echo off\n"${executable}" %*\n`);
      assert.equal(yield* find, shim);
      yield* fs.writeFileString(shim, `@echo off\n"${path.join(root, "other.exe")}" %*\n`);
      assert.equal(yield* find, undefined);
    }),
  );
