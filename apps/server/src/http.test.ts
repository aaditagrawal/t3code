...
describe("video asset byte ranges", () => {
  it.effect("streams exactly the requested bytes and leaves full downloads intact", () =>
    Effect.gen(function* () {
      ...
      const asset = { kind: "file" as const, path: file, contentType: "video/mp4" };
      ...
      const uppercase = HttpServerResponse.toWeb(
        yield* assetFileResponse({ ...asset, contentType: "Video/MP4" }, "bytes=0-1"),
      );
      ...
      const image = HttpServerResponse.toWeb(
        yield* assetFileResponse(
          { kind: "file", path: file, contentType: "image/png" },
          "bytes=0-1",
        ),
      );
    ...
    Effect.gen(function* () {
      ...
      for (const header of ["bytes=10-", "bytes=-0", "bytes=999999999999999999999999-"]) {
        const response = HttpServerResponse.toWeb(
          yield* assetFileResponse({ kind: "file", path: file, contentType: "video/mp4" }, header),
        );
      ...
      const empty = HttpServerResponse.toWeb(
        yield* assetFileResponse(
          { kind: "file", path: file, contentType: "video/mp4" },
          "bytes=0-1",
        ),
      );
  ...
  it("serves opaque attachments with their signed MIME and safe download disposition", () => {
    expect(
      ...
      headers: {
        "Cache-Control": "private, max-age=3600",
        "X-Content-Type-Options": "nosniff",
        "Content-Disposition": 'attachment; filename="release _notes_.pdf"',
