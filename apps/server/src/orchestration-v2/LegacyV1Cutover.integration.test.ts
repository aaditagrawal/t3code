 ...
 * through the fork's final V1 migration, 58. An application-local table is
 * retained without introducing a conflicting migration identity. The V2 runner
 * applies migrations 59 and 60 to the copied database.
 */
    ...
    () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          assert.isUndefined(
            boot1Logs.find((log) => String(log.message).includes("migration history diverges")),
          );
          assert.equal(firstBoot.migration41Name, "ProjectionThreadsPinned");
