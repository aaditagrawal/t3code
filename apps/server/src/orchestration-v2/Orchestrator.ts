...
import {
  HERMES_NOTIFICATION_RECORDS,
  planHermesNotificationDelivery,
} from "./HermesNotificationDelivery.ts";
...
const makeOrchestrator = Effect.fn("orchestrationV2.Orchestrator.layer")(function* () {
          ...
          return {
            ...
            ...(command.regenerateTitle === true
              ...
              : command.regenerateTitle === false || command.title !== undefined
                ? { titleRegeneration: null, titleRegenerationFailure: null }
                : {}),
            updatedAt: now,
          };
        }
        ...
        return yield* mapDispatchError(command)(
          planHermesNotificationDelivery({
            ...
            projection: yield* projectionStore
              .getThreadRecords(command.threadId, HERMES_NOTIFICATION_RECORDS)
              .pipe(mapDispatchError(command)),
            now: yield* DateTime.now,
            idAllocator,
          }),
  ...
  const dispatchWithReceiptEffect = Effect.fn("orchestrationV2.dispatch.withReceipt")(function* (
    ...
    const plan = yield* dispatchOnce(command).pipe(
      ...
      Effect.catch((cause) =>
        Effect.gen(function* () {
          // Hermes resolves Home before taking the destination thread lock.
          // Ownership can change in between; let its durable outbox retry the
          // side-effect-free notification plan instead of freezing rejection.
          if (command.type === "thread.notification.deliver") return yield* cause;
