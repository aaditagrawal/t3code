...
describe("CodexAdapterV2 runtime policy", () => {
  it.effect("derives concrete Codex turn policies from every T3 runtime mode", () =>
    Effect.gen(function* () {
      ...
      const mediumAccess = yield* build("medium-access");
      ...
      assert.equal(mediumAccess.approvalPolicy, "on-request");
      assert.equal(mediumAccess.approvalsReviewer, "user");
      assert.equal(mediumAccess.sandboxPolicy?.type, "workspaceWrite");
