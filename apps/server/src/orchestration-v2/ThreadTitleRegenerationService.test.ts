  ...
  it.effect("clears the marker and keeps the title when generation fails", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.equal(projection.thread.title, "Seed title");
        assert.isNotOk(projection.thread.titleRegeneration);
        assert.equal(projection.thread.titleRegenerationFailure?.requestId, requestId);
        assert.equal(
          projection.thread.titleRegenerationFailure?.error,
          "Could not regenerate the thread title. Try again.",
        );
        yield* armRegeneration({ command: "command:title:failure:retry", threadId });
        assert.isNotOk(
          (yield* threads.getThreadProjection(threadId)).thread.titleRegenerationFailure,
        );
      }).pipe(Effect.provide(harness.layer));
    }),
  );
