...
import {
  ChatAttachment,
  ThreadTitleRegenerationFailure,
...
interface LegacyThreadRow {
  ...
  readonly title_regeneration_failure_request_id: string | null;
  readonly title_regeneration_failure_at: string | null;
  readonly title_regeneration_failure_error: string | null;
...
function importedThread(row: LegacyThreadRow): OrchestrationV2AppThread {
  ...
  return {
    ...
    titleRegenerationFailure: Option.getOrNull(
      Schema.decodeUnknownOption(ThreadTitleRegenerationFailure)({
        requestId: row.title_regeneration_failure_request_id,
        failedAt: row.title_regeneration_failure_at,
        error: row.title_regeneration_failure_error,
      }),
    ),
  ...
  const reconcileShellsBase = Effect.gen(function* () {
    ...
    const repairRows = yield* sql<LegacyRepairRow>`
      SELECT
        ...
        thread.title_regeneration_failure_request_id,
        thread.title_regeneration_failure_at,
        thread.title_regeneration_failure_error,
        thread.deleted_at,
