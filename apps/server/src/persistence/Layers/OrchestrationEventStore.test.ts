    ...
    Effect.gen(function* () {
      ...
      const aggregateReplay = yield* Stream.runCollect(
        eventStore.readAggregateRange({
          aggregateKind: "thread",
          aggregateId: "thread-legacy-provider",
          fromSequenceExclusive: maxSequence,
          toSequenceInclusive: replayed[0]!.sequence,
          limit: 10,
        }),
      );
      assert.deepStrictEqual(aggregateReplay, replayed);
