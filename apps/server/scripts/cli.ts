 ...
 * every `@aaditagrawal/t3code-<platform>.tgz` first, `t3.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@aaditagrawal");
      const launcherTarball = path.join(scopeDir, "t3code.tgz");
      const platformTarballs = (yield* fs
        .readDirectory(scopeDir)
        .pipe(Effect.orElseSucceed((): ReadonlyArray<string> => [])))
        .filter((entry) => entry.startsWith("t3code-") && entry.endsWith(".tgz"))
        .sort()
        .map((entry) => path.join(scopeDir, entry));
      if (platformTarballs.length === 0) {
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(scopeDir, "t3code-<platform>.tgz"),
        });
      }
  ...
  Command.withDescription(
    "Publish the @aaditagrawal/t3code-<platform> tarballs and then the t3 launcher to npm.",
  ),
);
