...
const waitForRecordingStartupToSettle = async (recording: ActiveRecording): Promise<void> => {
  ...
  try {
    ...
    if (isBrowserRecordingStartCancelledError(cause)) return;
...
export async function startBrowserRecording(
  ...
  try {
    ...
    // Propagate the actual startup outcome. The stop path recognizes an
    // intentional queued cancellation without masking other failures.
    rejectStartup(error);
    throw error;
  } finally {
