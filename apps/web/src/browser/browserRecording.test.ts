...
describe("browser recording", () => {
  ...
  // A startup failure must reach a stop that is already waiting on startup,
  // instead of settling it as if the recording had started successfully.
  it("propagates screencast startup failures to a pending stop without saving", async () => {
    ...
    await Promise.resolve();
    // The stop parks on startup rather than cancelling the screencast.
    expect(stopScreencast).not.toHaveBeenCalled();
    const rejectedStop = expect(stopPromise).rejects.toMatchObject({
      operation: "wait-startup",
      tabId: "recording-tab",
  ...
  // The slot stays held after a stop times out, and is only released once the
  // abandoned start settles — here by rejecting rather than completing.
