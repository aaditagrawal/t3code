...
vi.mock("../../assets/assetUrls", async (importOriginal) => {
  const original = await importOriginal<typeof import("../../assets/assetUrls")>();
  return { ...original, useAssetUrls: vi.fn(original.useAssetUrls) };
});
...
describe("MessagesTimeline", () => {
  ...
  it("resolves assistant attachment URLs on the mounted row without history-wide previews", () => {
    const entry = buildAssistantTimelineEntry("Stored output");
    vi.mocked(useAssetUrls).mockReturnValue([
      "https://assets.example/chart-fresh.png",
      "https://assets.example/report-fresh.pdf",
    ]);
    try {
      const markup = renderToStaticMarkup(
        <MessagesTimeline
      ...
      expect(useAssetUrls).toHaveBeenLastCalledWith(ACTIVE_THREAD_ENVIRONMENT_ID, [
        { _tag: "attachment", attachmentId: "stored-image" },
        {
          _tag: "attachment",
          attachmentId: "stored-file",
          fileName: "report.pdf",
          mimeType: "application/pdf",
        },
      ]);
      expect(markup).toContain('src="https://assets.example/chart-fresh.png"');
      expect(markup).toContain('href="https://assets.example/report-fresh.pdf"');
      expect(markup).toContain('download="report.pdf"');
    } finally {
      vi.mocked(useAssetUrls).mockReset();
    }
  });
  ...
  it("renders proactive assistant image and file attachments", () => {
            ...
            message: {
              ...
              attachments: [
                ...
                },
                {
                  ...
                  mimeType: "video/mp4",
                  sizeBytes: 30,
                  previewUrl: "https://assets.example/demo.mp4",
                },
              ],
            },
          },
    ...
    expect(markup).not.toContain('download="demo.mp4"');
  });
