...
export function ContextWindowMeter(props: {
  ...
  const usageColor = isOverloaded ? "var(--color-error)" : "var(--color-usage-meter)";
      ...
      >
        <div className="flex flex-col gap-2 p-(--floating-content-inset)">
          ...
          {usage.maxTokens !== null ? (
            ...
            >
              <div
                className="h-full rounded-full transition-[width,background-color] duration-500 ease-out motion-reduce:transition-none w-(--width) bg-(--background-color)"
                style={{ "--width": `${normalizedPercentage}%`, "--background-color": usageColor }}
              />
            </div>
          ) : null}
          {showTotalProcessed ? (
